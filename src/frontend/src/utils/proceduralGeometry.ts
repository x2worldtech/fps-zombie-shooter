 ...
 */
export function generateMountainCollisionAABBs(
  playerRadius = 0.4,
): CollisionAABB[] {
...
export const PALM_TREE_POSITIONS: [number, number, number][] =
  generatePalmTreePositions();
...
export function getValidJuggernogPosition(): [number, number, number] {
  ...
  for (const [x, z] of candidates) {
    ...
    if (Math.sqrt((x - papX) ** 2 + (z - papZ) ** 2) < papExclusionRadius)
      continue;
...
export const JUGGERNOG_POSITION: [number, number, number] =
  getValidJuggernogPosition();
