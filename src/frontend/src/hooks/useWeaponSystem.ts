...
export function useWeaponSystem() {
  ...
  const reload = useCallback(() => {
    ...
    if (state.currentAmmo === WEAPON_CONFIGS[state.currentWeapon].magazineSize)
      return;
  ...
  const upgradeWeapon = useCallback(
    (currentPoints: number): { success: boolean; cost: number } => {
      const state = stateRef.current;
      ...
      setWeaponState((prev) => ({
        ...prev,
        upgradeTier: prev.upgradeTier + 1,
      }));
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      const nextIdx =
        (currentIdx + dir + WEAPON_ORDER.length) % WEAPON_ORDER.length;
