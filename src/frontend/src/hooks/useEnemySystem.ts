...
import { type Enemy, EnemyType, type Pickup } from "../types/enemy";
...
export function useEnemySystem() {
      ...
      setEnemies((prev) =>
        prev.map((e) => {
          ...
          if (dist > attackRange) {
            ...
            return {
              ...e,
              position: [newX, e.position[1], newZ] as [number, number, number],
            };
          }
  ...
  const clearDeadEnemies = useCallback(() => {
    const now = Date.now();
    setEnemies((prev) =>
      prev.filter((e) => !e.isDead || now - e.deathTime < 1200),
    );
