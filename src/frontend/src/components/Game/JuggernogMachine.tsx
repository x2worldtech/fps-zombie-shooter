...
export default function JuggernogMachine({
  position,
  purchaseCount,
  ...
  const bodyMaterial = useMemo(
    () => new THREE.MeshToonMaterial({ color: "#cc1111" }),
    [],
  );
  const darkRedMaterial = useMemo(
    () => new THREE.MeshToonMaterial({ color: "#880000" }),
    [],
  );
  const chromeMaterial = useMemo(
    () => new THREE.MeshToonMaterial({ color: "#aaaaaa" }),
    [],
  );
  const blackMaterial = useMemo(
    () => new THREE.MeshToonMaterial({ color: "#111111" }),
    [],
  );
